...
def preproc(data_pred):
    ...
    genres_cols = list(set(genres_test.columns))
...
def make_data():
    ...
    df2 = {'id': np.max(data.id)+1, 'book_title': book_title, 'book_image_url': book_image_url,
...
def make_predict(pred_id):
    ...
    if value:
        with st.spinner('Очікуйте результати...'):
            ...
            preproc_value = preproc_values.iloc[pred_id, :]
