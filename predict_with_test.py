...
def make_predict(pred_id):
    ...
    if value:
        with st.spinner('Очікуйте результати...'):
            ...
            data['url_shape'] = test_info['url_shape']
            data['url_shape'] = data['url_shape'].apply(lambda x: eval(x))
            
            data['height'] = data['url_shape'].apply(lambda x: x[0] if x else None)
            data['width'] = data['url_shape'].apply(lambda x: x[1] if x else None)
            data.drop(columns=['url_shape'], inplace=True)
